 ...
 *
 * @param Confirm Speed
 * @desc 画像を選択したときのアニメーションの速度。
 * @default 0.2
 *
 * @param Close Speed
 * @desc 画像を閉じるときのアニメーションの速度。
 * @default 0.4
 *
 * @help
 *
...
Window_PictureChoiceList.prototype.windowHeight = function() {
	if(!this._messageWindow.isClosed()) {
        ...
        if ($gameMessage.pictureChoiceGrid[1]==2) {
            return 300;
	} else {
            return 540;
	}
	//変更　return $gameMessage.pictureChoiceSize[1];
};
...
Sprite_PictureChoice.prototype.initialize = function() {
	...
	this.anchor.x = -2.0;//0.5; //変更　選択肢の位置
	this.anchor.y = 0.5; //変更　0;//
	this._breath = 1;
	this._direction = 0;
};
