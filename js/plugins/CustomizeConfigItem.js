    ...
    Window_Options.prototype.addCustomOptions = function() {
        iterate(this._customParams, function(key, item) {
            if (!ConfigManager.hiddenInfo[key]) {
             ...
             //if (item.name == '断面図' && !$gameSwitches.value(115)) {
             //   if (!ConfigManager.hiddenInfo[key]) this.addCommand('Sectional view', key);
             //} else if (item.name == 'エッチ時のセリフ' && !$gameSwitches.value(115)) {
             //   if (!ConfigManager.hiddenInfo[key]) this.addCommand('H text', key);
             //} else if (item.name == '軽量化' && !$gameSwitches.value(115)) {
             //   if (!ConfigManager.hiddenInfo[key]) this.addCommand('Lightweight', key);
             //} else {
